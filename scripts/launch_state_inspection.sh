...
ENV="+env=base +env/opt=base +env/reward=ep_done_scaled +env/obs=all"
BASERUNDIR='baserundir=runs_statespace/${action_space_id}'

# 1
python -m $BASE $SEED $TASKALL +eval=base $ENV $WEI_ACT $POLICY_DEFAULT $CLUSTER $BASERUNDIR --multirun &
