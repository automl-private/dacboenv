...
def ros(x):
    ...
    # Here, we maximize
    x_new = x.reshape(-1, 2).copy()
    return -np.array(problem(x_new)).reshape(x_new.shape[0], 1)

def ros_var(x, t, s):
    """
    x : array of shape (2,) or (N, 2)
    t : array-like of shape (2,)
    s : scalar
    """
    x = np.asarray(x)
    t = np.asarray(t)
    ...
    # Compute per-point translation bounds
    # lower bound: -5 - x
    # upper bound:  5 - x
    t_min = -5.0 - x_new
    t_max =  5.0 - x_new

    # Clip translation for each point and dimension
    t_clipped = np.clip(t, t_min, t_max)

    # Apply translation
    x_new = x_new - t_clipped

    # Evaluate Rosenbrock (vectorized)
    values = np.apply_along_axis(ros, 1, x_new).reshape(-1, 1)
...
def ros_max_min():
    problem = ioh.get_problem(fid=8, dimension=2, instance=0)
    max_pos = np.array(problem.optimum.x).reshape(-1, 2)
    max_val = -problem.optimum.y

    min_pos = np.array([[5.0, -5.0]])
    min_val = ros(min_pos[0])

    return max_pos, max_val, min_pos, min_val

def ros_max_min_var(t, s):
    max_pos, max_val, min_pos, min_val = ros_max_min()

    # translation bounds so points stay in [-5, 5]^2
    t_range = np.array([
        [-5.0 - max_pos[0, 0],  5.0 - max_pos[0, 0]],
        [-5.0 - max_pos[0, 1],  5.0 - max_pos[0, 1]]
    ])
    ...
    return max_pos, s * max_val, min_pos, s * min_val
