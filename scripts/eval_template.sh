...
ENV="+eval=base +env=base +env/obs=smart +env/reward=ep_done_scaled +env/opt=base +cluster=cpu_noctua seed=range(1,11) -m"

PPO_BASE="+policy/optimized/PPO/PPO-Perceptron-dacbo_Cepisode_length_scaled_plus_logregret"
...
UCB_P2="+env/action=ucb_beta_continuous ${PPO_BASE}${UCB_SUFFIX}_3seeds=glob(*)"
WEI_P2="+env/action=wei_alpha_continuous ${PPO_BASE}${WEI_SUFFIX}_3seeds=glob(*)"
...
for fid in {1..24}; do
    for d in 2 8; do
        run_eval "+task/BBOB=cfg_${d}_${fid}_0" \
                 ...
                 "${PPO_BASE}${UCB_SUFFIX}_fid${fid}_3seeds=glob(*)"
        
        run_eval "+task/BBOB=cfg_${d}_${fid}_0" \
                 "+env/action=wei_alpha_continuous" \
                 "${PPO_BASE}${WEI_SUFFIX}_fid${fid}_3seeds=glob(*)"
    done
done
