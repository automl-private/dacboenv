...
from metabo.environment.objectives import SparseSpectrumGP, bra_var, bra_max_min_var, gprice_var, gprice_max_min_var, \
    ...
    rhino2, rhino2_max_min, ros_var, ros_max_min_var
from metabo.environment.furuta import init_furuta_simulation, furuta_simulation
from matplotlib import pyplot as plt
import os

from metabo.environment.objectives_bbob import (
    BBOBFunction,
    TranslatedScaledBBOB,
    bbob_max_min_var
)
...
class MetaBO(gym.Env):
    ...
    def draw_new_function(self):
        if self.f_type == "GP":
        ...
        elif self.f_type in {"BRA-var", "ROS-var"} or "bbob" in self.f_type.lower() or "yahpo" in self.f_type.lower():
            ...
            if "M" in self.f_opts and self.f_opts["M"] is not None:
                ...
                fct_params_domain = np.vstack([
                    np.tile([-bound_translation, bound_translation], (self.D, 1)),
                    [[1 - bound_scaling, 1 + bound_scaling]]
                ])

                fct_params_grid = sobol_seq.i4_sobol_generate(
                    dim_num=self.D + 1, n=M
                )

                param_idx = self.rng.choice(M)

                t = fct_params_grid[param_idx, :self.D]
                s = fct_params_grid[param_idx, self.D]
            else:
                if "bound_translation" in self.f_opts:
                    # sample translation
                    t = self.rng.uniform(low=-self.f_opts["bound_translation"],
                                         high=self.f_opts["bound_translation"], size=(self.D,))
                    ...
                    t = self.rng.uniform(low=self.f_opts["translation_min"],
                                         high=self.f_opts["translation_max"], size=(self.D,))
            ...
            if self.f_type == "BRA-var":
                self.f = lambda x: bra_var(x, t=t, s=s)
                max_pos, max, _, min = bra_max_min_var(t=t, s=s)
            elif self.f_type == "ROS-var":
                self.f = lambda x: ros_var(x, t=t, s=s)
                max_pos, max, _, min = ros_max_min_var(t=t, s=s)
            elif "bbob" in self.f_type.lower():
                fid = self.f_opts["fid"]
                dim = self.D
                log_path = self.f_opts.get("log_path", None)
                fid_instances = self.f_opts.get("fid_instances", None)
                if fid_instances:
                    fid = fid_instances[(self.running_t // self.T) % len(fid_instances)]
                base_fn = BBOBFunction(fid=fid, dimension=dim, log_path=log_path, T=self.T)
                self.f = lambda x: TranslatedScaledBBOB(base_fn)(x, t=t, s=s)
                t_vec = t.reshape(-1)
                max_pos, max, _, min = bbob_max_min_var(fid=fid, dimension=dim, t=t_vec, s=s)
            elif "yahpo" in self.f_type.lower():
                dataset = self.f_opts["dataset"]
                name = self.f_opts["benchmark_name"]
                base_fn = YAHPOFunction(benchmark_name=name, dataset_name=dataset)
                self.f = lambda x: TranslatedScaledYAHPO(base_fn)(x, t=t, s=s)
                max_pos, max_val, min_pos, min_val = yahpo_max_min_var(base_fn, t, s)
            self.x_max = max_pos
            self.y_max = max
            self.y_min = min
