...
class DACBOEnvOptimizer(SMAC3Optimizer):
    ...
    def __init__(self, task, smac_cfg, loggers = None, expects_multiple_objectives = False, expects_fidelities = False, frequency = 10):
    ...
    def ask(self):
        ...
        if len(self.solver.runhistory) % self._frequency == 0 and len(self.solver.runhistory) > len(self.solver.intensifier.config_selector._initial_design_configs):
    ...
    def tell(self, trial_info, trial_value):
        ...
        obs, _ = DACBOEnv.get_observation(self.solver.optimizer)
        self._state = obs